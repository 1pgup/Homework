...
int credit_limit_change(FILE *rec_ptr, FILE *trans_ptr, FILE *cr_lim_ch_ptr, Data_t client, Data_t transfer) {
    while (fscanf(rec_ptr, "%d%19s%19s%29s%14s%lf%lf%lf",
                        &client.number,
                        client.name,
                        client.surname,
                        client.address,
        ...
        /*if (fscanf(rec_ptr, "%d%19s%19s%29s%14s%lf%lf%lf",
                        &client.number,
                        client.name,
                        client.surname,
                        client.address,
        ...
        while (fscanf(trans_ptr, "%d %lf", &transfer.number, &transfer.cash_payments) !=  -1) {
            /*if (fscanf(trans_ptr, "%d %lf", &transfer.number, &transfer.cash_payments) !=  2) {
                return ERROR_RETURN;
            } */
...
int client_inf_write(FILE *rec_ptr, Data_t client) {
    ...
    while (scanf("%d%19s%19s%29s%14s%lf%lf%lf",
                ...
                &client.credit_limit,
                &client.cash_payments) != -1) {
        /*if (scanf("%d%19s%19s%29s%14s%lf%lf%lf",
                &client.number,
                client.name,
                client.surname,
                client.address,
...
int transaction_write(FILE *trans_ptr, Data_t transfer) {
    ...
    while (scanf("%d%lf", &transfer.number, &transfer.cash_payments) != -1) {
        /*if (scanf("%d%lf", &transfer.number, &transfer.cash_payments) != 2) {
            return ERROR_RETURN;
        }*/
